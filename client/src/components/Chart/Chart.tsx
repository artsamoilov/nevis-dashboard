            ...
            <div role="img" aria-label={description}>
                <ResponsiveContainer width="100%" height={320}>
                    <BarChart data={data} margin={CHART_MARGIN} barCategoryGap={BAR_CATEGORY_GAP}>
                        <CartesianGrid vertical={false} stroke={GRIDLINE_COLOR} strokeDasharray="2 4"/>
                        <XAxis dataKey="month" tickLine={false} axisLine={false} tick={axisTickStyle}/>
                        <YAxis tickLine={false} axisLine={false} allowDecimals={false} tick={axisTickStyle}/>
                        <Legend iconType="square" iconSize={8} wrapperStyle={legendWrapperStyle} labelStyle={legendLabelStyle}/>
                        {series.map((child, i) => (
                            <Bar
                                key={child.id}
                                dataKey={child.name}
                                ...
                                barSize={BAR_SIZE}
                                shape={(props) => <Rectangle {...props} radius={getSegmentRadius(props.payload, seriesNames, i)}/>}
                            />
                        ))}
                    </BarChart>
                </ResponsiveContainer>
