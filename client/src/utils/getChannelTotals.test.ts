...
const channel = (name: string, values: number[]): ITreeNode => ({id: name, name, values, variant: NODE_TYPE.CHANNEL});
...
describe("getChannelTotals", () => {
    it("sums the same channel name across every employee that has it", () => {
        const tree = branch("Branch 1", [
            employee("Anna", [channel("Existing clients", [10, 12]), channel("New organic", [1, 2])]),
