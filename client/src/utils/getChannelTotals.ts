...
export const getChannelTotals = (node: ITreeNode): ITreeNode => {
    ...
    for (const channel of findChannels(node)) {
        ...
        if (existing) existing.values = existing.values.map((v, i) => v + channel.values[i]);
        else channels.push({...channel});
    }

    return {id: "channel-totals", name: "Acquisition channels", variant: NODE_TYPE.CHANNEL, values: node.values, children: channels};
};
